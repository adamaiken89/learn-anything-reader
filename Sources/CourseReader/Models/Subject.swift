  ...
  static func parse(yaml: String, directory: String) -> Subject? {
    ...
    for line in lines {
      ...
      if trimmed.hasPrefix("subject:") {
        ...
        inObjectives = false; inPrereqs = false; inModules = false
        continue
      }
      ...
      if trimmed.hasPrefix("prerequisites:") {
        inPrereqs = true; inObjectives = false; inModules = false
        continue
      }
      if trimmed.hasPrefix("learning_objectives:") {
        inObjectives = true; inPrereqs = false; inModules = false
        continue
      }
      if trimmed.hasPrefix("modules:") {
        inModules = true; inObjectives = false; inPrereqs = false
        continue
      }

      if inPrereqs, trimmed.hasPrefix("-") {
        prereqs.append(String(trimmed.dropFirst(1)).trimmingCharacters(in: .whitespaces).trimmingCharacters(in: CharacterSet(charactersIn: "\"'")))
        continue
      }

      if inObjectives, trimmed.hasPrefix("-") {
        objectives.append(String(trimmed.dropFirst(1)).trimmingCharacters(in: .whitespaces).trimmingCharacters(in: CharacterSet(charactersIn: "\"'")))
        continue
      }
      ...
      if inModules {
        if trimmed.hasPrefix("- id:") {
          if let m = currentModule {
            modules.append(ModuleMeta(id: m.id, name: m.name, timeHours: m.hours, prerequisites: m.prereqs, topics: m.topics))
          }
          ...
          if trimmed.hasPrefix("name:") {
            let name = String(trimmed.dropFirst(5)).trimmingCharacters(in: .whitespaces).trimmingCharacters(in: CharacterSet(charactersIn: "\"'"))
            currentModule = (m.id, name, m.hours, m.prereqs, m.topics)
          } else if trimmed.hasPrefix("time_hours:") {
            let hours = Double(String(trimmed.dropFirst(11)).trimmingCharacters(in: .whitespaces)) ?? 0
            ...
            if prereqStr.hasPrefix("[") {
              ...
              let nums = cleaned.components(separatedBy: ",").compactMap { Int($0.trimmingCharacters(in: .whitespaces)) }
            ...
            if topicStr.hasPrefix("[") {
              ...
              let topics = cleaned.components(separatedBy: ",").map { $0.trimmingCharacters(in: .whitespaces).trimmingCharacters(in: CharacterSet(charactersIn: "\"'")) }
