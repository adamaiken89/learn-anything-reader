...
private func parseQuizYAML(_ yaml: String) -> [QuizQuestion] {
  ...
  for line in lines {
    ...
    if trimmed.hasPrefix("- id:") {
      ...
      currentId = String(trimmed.dropFirst(5)).trimmingCharacters(in: .whitespaces)
    ...
    if trimmed.hasPrefix("question:") {
      currentQ = String(trimmed.dropFirst(9)).trimmingCharacters(in: .whitespaces)
        .trimmingCharacters(in: CharacterSet(charactersIn: "\"'"))
      inOptions = false
      inTags = false
      ...
      currentExplanation = String(trimmed.dropFirst(12)).trimmingCharacters(in: .whitespaces)
        .trimmingCharacters(in: CharacterSet(charactersIn: "\"'"))
      inOptions = false
      inTags = false
    } else if trimmed.hasPrefix("difficulty:") {
      currentDifficulty =
        Int(String(trimmed.dropFirst(11)).trimmingCharacters(in: .whitespaces)) ?? 1
      inOptions = false
      inTags = false
      ...
      if rest.hasPrefix("[") {
        ...
        currentTags = cleaned.components(separatedBy: ",").map {
