...
export default function CumulativeQuizSection({ course, cumulativeQuizId }: Props) {
  ...
  useEffect(() => {
    api.quiz
      .cumulative(course.id, cumulativeQuizId)
      .then((cq) => {
  ...
  const hasAnswer =
    currentIndex < questions.length && selectedAnswers[questions[currentIndex]?.id] !== undefined;
          ...
          <div className="flex gap-2 mb-6 justify-center">
            {(['all', 'wrong', 'correct'] as const).map((f) => {
              ...
              return (
                ...
                >
                  {t(`quiz.filter${f.charAt(0).toUpperCase() + f.slice(1)}`, { count })}
                </button>
              );
            })}
          </div>
